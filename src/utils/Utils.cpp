...
ImageBase Utils::resizeImage(std::vector<unsigned char> image,
                             int imagesPerSide, int imagettesSize) {
  int wantedSize =
      imagesPerSide * imagesPerSide * imagettesSize * imagettesSize;
    ...
    for (int x = 0; x < sideImage; x++) {
      for (int yi = 0; yi < diff; yi++) {
        for (int xi = 0; xi < diff; xi++) {
          imageOUT[yi + diff * y][xi + diff * x] =
              (int)image[y * sideImage + x];
        }
      }
    }
  }
