...
ImageBase::ImageBase(const ImageBase &other) { copy(other); }
...
void ImageBase::load(const char *filename) {
  ...
  {
    printf("Chargement de l'image impossible : Le nom de fichier n'est pas "
           "conforme, il doit comporter l'extension, et celle ci ne peut �tre "
           "que '.pgm' ou '.ppm'");
    exit(0);
  }
...
bool ImageBase::save(const char *filename) {
  if (!isValid) {
    printf(
        "Sauvegarde de l'image impossible : L'image courante n'est pas valide");
    exit(0);
  }
...
ImageBase *ImageBase::getPlan(PLAN plan) {
  ...
  case PLAN_B:
    ...
    break;
  default:
    printf("Il n'y a que 3 plans, les valeurs possibles ne sont donc que "
